	...
	return (
		<header className="w-full border-b border-zinc-800 bg-zinc-950/80 backdrop-blur-md sticky top-0 z-50">
			<div className="max-w-5xl mx-auto flex items-center justify-between p-4">
				<Link href="/" className="font-bold text-lg tracking-tight text-zinc-50">
					DH.
				</Link>
				...
				<nav className="hidden md:flex gap-6 text-sm font-medium text-zinc-400">
					...
					<Link href="/contact" className="hover:text-zinc-50 transition-colors">
						Contact
					</Link>
				</nav>
			...
			{isOpen && (
				<nav className="md:hidden border-t border-zinc-900 bg-zinc-950 p-4 flex flex-col gap-4 text-sm font-medium text-zinc-400">
					<Link href="/projects" onClick={() => setIsOpen(false)} className="hover:text-zinc-50">
						Projects
					</Link>
					<Link href="/contact" onClick={() => setIsOpen(false)} className="hover:text-zinc-50">
						Contact
					</Link>
				</nav>
			)}
